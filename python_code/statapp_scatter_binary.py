...
if __name__ == "__main__":
    ...
    if SCATTER_MATRIX_CURRENTS:           
        ...
        sns.set()
        pal = sns.color_palette(palette='Set1',n_colors=2)       
        g = sns.pairplot(df,hue='label',palette=pal,vars=col_labels[:-1])
        g.set(yticks=[])
        g.set(xticks=[])
        new_labels = ['faulty','working']
        for t, l in zip(g._legend.texts, new_labels): 
            t.set_text(l)
        plt.savefig(os.path.join('imgs','scattermatrix_currents_bin.pdf'),format='pdf',dpi=1200)
    ...
    if SCATTER_MATRIX_TORQUES:
        ...
        for t, l in zip(g._legend.texts, new_labels): 
            t.set_text(l)
        plt.savefig(os.path.join('imgs','scattermatrix_torques_bin.pdf'),format='pdf',dpi=1200)          
